...
def check_env_vars(var_name: str, default: Optional[str] = None) -> str:
...
def display_summary_statistics(df_summary: pd.DataFrame) -> None:
    ...
    print(f"Total read volume: {readable_read}")
    print(f"Total write volume: {readable_write}")
    print(f"Total IO volume: {readable_total}")
    
    # Per-user statistics if there are multiple users
    if len(df_summary["user_name"].unique()) > 1:
        print("\nIO Metrics by User:")
        user_stats = df_summary.groupby("user_name").agg({
            "total_read_bytes": "sum",
            "total_write_bytes": "sum",
            "workflow_id": "count"
        }).reset_index()
        
        user_stats["total_io_bytes"] = user_stats["total_read_bytes"] + user_stats["total_write_bytes"]
        ...
        for _, row in user_stats.iterrows():
            ...
            print(f"  Read: {read_bytes}")
            print(f"  Write: {write_bytes}")
            print(f"  Total: {total_bytes}")


@app.command()
def main(
    from_date: datetime = typer.Option(..., "--from", help="Earliest date to consider (inclusive) in YYYY-MM-DD format"),
    to_date: Optional[datetime] = typer.Option(None, "--to", help="Last date to consider (inclusive) in YYYY-MM-DD format"),
    output: str = typer.Option("io_metrics.csv", "--output", "-o", help="Output CSV file path"),
    workspace_id: Optional[str] = typer.Option(None, "--workspace-id", "-w", help="Specific workspace ID to analyze"),
    user: Optional[str] = typer.Option(None, "--user", "-u", help="Filter workflows by user"),
    status: str = typer.Option("SUCCEEDED", "--status", "-s", help="Filter workflows by status (SUCCEEDED, FAILED, etc.)"),
    endpoint: Optional[str] = typer.Option(None, "--endpoint", "-e", help="API endpoint URL (defaults to environment variable)")
) -> None:
    ...
    # API setup
    base_url = endpoint if endpoint else check_env_vars("TOWER_API_ENDPOINT", "https://api.cloud.seqera.io")
    ...
    if workspace_id:
        ...
        summaries, process_metrics = process_workspace(
            client, workspace_id, min_time, max_time, user, status, org_name, workspace_name
        )
        ...
        if orgs:
            for org in orgs.get("organizations", []):
                ...
                for workspace in workspaces:
                    ...
                    summaries, process_metrics = process_workspace(
                        client, workspace_id, min_time, max_time, user, status, 
